...
public class DAODeptManager implements DAO<DeptManager>{
	public static String TABLE_NAME = "dept_manager";
	...
	public boolean addItem(DeptManager item) {
		...
		try {
			s = DatabaseAccess.getInstance().getConnection()
					.prepareStatement("INSERT INTO " + DAODeptManager.TABLE_NAME + " VALUES(?,?,?,?);");
			PreparedStatement maxKey = DatabaseAccess.getInstance().getConnection()
					.prepareStatement("SELECT MAX(" + DAODeptManager.COLUMNS[0] + ") FROM " + DAODeptManager.TABLE_NAME + ";");
	...
	public boolean removeItem(DeptManager item) {
		...
		try {
			s = DatabaseAccess.getInstance().getConnection().prepareStatement(
					"DELETE FROM " + DAODeptManager.TABLE_NAME + " WHERE " + DAODeptManager.COLUMNS[0] + " = ?;");
	...
	public boolean modifyItem(DeptManager item) {
		...
		try {
			s = DatabaseAccess.getInstance().getConnection()
					.prepareStatement("UPDATE " + DAODeptManager.TABLE_NAME + " SET " + DAODeptManager.COLUMNS[1] + "=?,"
							+ DAODeptManager.COLUMNS[2] + "=?," + DAODeptManager.COLUMNS[3] + "=? WHERE " + DAODeptManager.COLUMNS[0] + " = ?;");
	...
	public ArrayList<DeptManager> createList(int numRows) {
		...
		try {
			ResultSet r = null;
			if (numRows == -1) {
				r = DatabaseAccess.getInstance().getConnection().prepareStatement("SELECT * FROM " + TABLE_NAME + ";")
						.executeQuery();
			} else {
				r = DatabaseAccess.getInstance().getConnection()
						.prepareStatement("SELECT * FROM " + DAODeptManager.TABLE_NAME +";").executeQuery();
			}
			if (r != null) {
